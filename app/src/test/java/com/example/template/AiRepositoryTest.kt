...
class AiRepositoryTest {
  ...
  fun `analyze returns Cloud on successful API response`() =
    runBlocking {
      ...
      assertEquals("AI generated response", (result as AiResult.Cloud).text)
    }
  ...
  fun `geminiConfig has correct base url`() {
    assertEquals("https://generativelanguage.googleapis.com/v1beta/", GeminiConfig.BASE_URL)
  }
}
      ...
      candidates =
        listOf(
          GeminiResponse.Candidate(
            content = GeminiResponse.Content(parts = listOf(GeminiResponse.Part(text = responseText)))
          )
        )
    )
  }
